...
| Store and recovery | queued text survives restart without accidental speech | SQLite-backed store; architecture §§3 and 6 | reopen/recovery examples plus seeded commit-failure rollback | expand to crash-point and recovery-time fault matrices |
...
| Synthesis and cache | failures isolate; ready audio is usable; disk remains bounded | synthesis/artifact/cache ports; architecture §§2, 4, and 6 | generated LRU model, real filesystem contract, seeded missing/partial-write/cleanup faults | add crash-safe atomic publication and real full-disk integration |
...
- a synthesis-artifact port with seeded missing-output, partial-write, and
  stacked cleanup failures proving only usable audio becomes Ready and cleanup
  faults cannot stop the worker pool.

Still open for v0.1.0 readiness where the product makes the corresponding
promise:

- crash-point store recovery beyond commit rollback and crash-safe atomic audio
  publication;
