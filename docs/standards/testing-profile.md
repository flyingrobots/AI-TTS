...
| Synthesis and cache | failures isolate; ready audio is usable; disk remains bounded | synthesis pool/cache boundary; architecture §§2, 4, and 6 | fake-engine lifecycle examples | bounded eviction is not implemented; add fault injection for writes/full disk |
...
- seeded store/cache fault injection for persistence and recovery claims;
- systematic playback interleaving exploration beyond named regressions;
- cache-cap enforcement and its eviction invariants;
